...
periods 7-26 and evaluate on period 27. All commands below are run from the
repo root.
...
`output/ppo/training/ongoing/{out_prefix}ppo_seed_{seed}{out_suffix}/`.
Evaluation outputs are written to `output/ppo/testing/{run_name}/`.
...
- `--obs_type`: choose the observation schema JSON in `ppo/configs/`.
...
```bash
python ppo/main_eval_ppo.py \
  --load_path output/ppo/training/ongoing/001_temporal8_ppo_seed_0_ppo_default_obs16 \
  --eval_mode both \
  --n_eval_episodes 100 \
  --eval_period 27 \
  --obs_type obs_16_keys \
...
```bash
python ppo/main_eval_ppo.py \
  --load_path output/ppo/training/ongoing/003_t30_ppo_seed_0_ppo_default_obs16 \
  --eval_mode both \
  --n_eval_episodes 100 \
  --eval_period 27 \
  --obs_type obs_16_keys \
...
```bash
python ppo/main_eval_ppo.py \
  --load_path output/ppo/training/ongoing/002_lambda0.5_ppo_seed_0_ppo_default_obs16 \
  --eval_mode both \
  --n_eval_episodes 100 \
  --eval_period 27 \
  --obs_type obs_16_keys \
...
```bash
python ppo/main_eval_ppo.py \
  --load_path output/ppo/training/ongoing/022_full_obs_market_regime_v1_ppo_seed_0_5m_all \
  --eval_mode both \
  --n_eval_episodes 100 \
  --eval_period 27 \
  --obs_type obs_market_regime_v1 \
